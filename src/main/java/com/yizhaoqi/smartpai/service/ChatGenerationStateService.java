...
public class ChatGenerationStateService {
    ...
    private final Map<String, GenerationSnapshot> memoryGenerations = new ConcurrentHashMap<>();
    private final Map<String, String> memoryActive = new ConcurrentHashMap<>();
    ...
    public GenerationSnapshot createGeneration(String userId, String conversationId, String question) {
        ...
        if (inMemory) {
            GenerationSnapshot snapshot = toSnapshot(meta, "", Collections.emptyMap());
            memoryGenerations.put(generationId, snapshot);
            memoryActive.put(userId, generationId);
            return snapshot;
        }
    ...
    public void appendChunk(String generationId, String chunk) {
        if (inMemory) {
            memoryGenerations.computeIfPresent(generationId, (id, old) -> new GenerationSnapshot(
                    old.generationId(), old.userId(), old.conversationId(), old.question(), old.status(),
                    old.content() + (chunk == null ? "" : chunk), old.createdAt(), LocalDateTime.now().toString(),
                    old.errorMessage(), old.referenceMappings()));
            return;
        }
    ...
    public void updateReferenceMappings(String generationId, Map<String, Map<String, Object>> referenceMappings) {
        if (inMemory) {
            memoryGenerations.computeIfPresent(generationId, (id, old) -> new GenerationSnapshot(
                    old.generationId(), old.userId(), old.conversationId(), old.question(), old.status(), old.content(),
                    old.createdAt(), LocalDateTime.now().toString(), old.errorMessage(),
                    referenceMappings == null ? Collections.emptyMap() : Map.copyOf(referenceMappings)));
            return;
        }
        try {
    ...
    public Optional<GenerationSnapshot> getGeneration(String generationId) {
        if (inMemory) return Optional.ofNullable(memoryGenerations.get(generationId));
    ...
    public Optional<GenerationSnapshot> getActiveGenerationForUser(String userId) {
        if (inMemory) {
            String id = memoryActive.get(userId);
            return id == null ? Optional.empty() : getGenerationForUser(id, userId);
        }
    ...
    private void updateTerminalState(String generationId,
        ...
        if (inMemory) {
            memoryGenerations.computeIfPresent(generationId, (id, old) -> {
                memoryActive.remove(old.userId(), generationId);
                return new GenerationSnapshot(old.generationId(), old.userId(), old.conversationId(), old.question(),
                        status, old.content(), old.createdAt(), LocalDateTime.now().toString(), errorMessage,
                        referenceMappings == null ? old.referenceMappings() : Map.copyOf(referenceMappings));
            });
            return;
        }
        GenerationMeta meta = readMeta(generationId);
